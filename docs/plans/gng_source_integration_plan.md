...
15. `T10.7` Tracker Continuity Fix *(remediation — unblocks T11.4 calibration quality)*
    ...
    - `T10.7 ST5` ✅ Propagate all calibrated values to mechanics YAML + regenerate episodes *(complete: mechanics synced; 7 episodes regenerated with ≥1 `jump_start` each)*
...
- `T10.7 ST5` completed after T10.7.C and T10.7.E landed. `specs/mechanics/gng_abstract_mechanics.yaml` now matches the calibration YAML for all four constants, and `specs/episodes/gng_episodes.json` was regenerated from the current trace with 7 qualifying episodes (each containing at least one `jump_start`).
